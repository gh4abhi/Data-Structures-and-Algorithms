...
void DFSForBridges(ll src, ll par, vector<ll> &tin, vector<ll> &low, vector<ll> &vis, vector<vector<ll>> &adj, vector<vector<ll>> &ans, ll timer)
    {
        vis[src] = 1;
        ...
        for(auto i:adj[src])
        {
            ...
            if(vis[i]==0)
            {
                DFSForBridges(i,src,tin,low,vis,adj,ans,timer);
...
vector<vector<int>> findBridges(int n, vector<vector<int>>& edges) {
        ...
        for(ll i=0;i<n;i++)
            if(vis[i]==0)
                DFSForBridges(i,-1,tin,low,vis,adj,ans,timer);
        return ans;
    }

//--------------------------Articulation Point in Graph------------------------------------

    void DFS(ll src, ll par, ll timer, vector<ll> &tin, vector<ll> &low, vector<ll> &vis, vector<ll> &ans, vector<vector<ll>> adj)
    {
        vis[src] =1;
        ll child = 0;
        tin[src] = low[src] = timer++;
    ...
    vector<int> articulationPoints(int V, vector<vector<ll>> adj) {
        vector<ll> tin(V,-1);
        vector<ll> low(V,-1);
        vector<ll> vis(V,0);
        vector<ll> ans(V,0);
